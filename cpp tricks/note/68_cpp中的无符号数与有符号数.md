...
- 2.**有符号数转为无符号数**
    ...
    ```
- 解释：当有符号数和无符号数相加时，首先将有符号数变为无符号数，然后执行加法操作。有符号数转为无符号数的方法：有符号数+无符号数的模。例如上例中，int i = -42;i是一个有符号数，unsigned int u是一个无符号数，u在32位机器上占4个字节，即32位，故无符号数u的模是2^32。所以，最后的结果是u+i =-42+2^32+10
- 3.**两个无符号数之间的运算**
```
